    ...
    public class OptimizeDependencies : IBuildStep
    {
        ...
        public void Start(BuildTask task)
        {
            ...
            foreach (var entry in assets)
            {
                ...
                // 按引用关系打包，没有产生新分组引用关系时，自动分组的 bundle 名字不会变
                var groups = new List<string>();
                foreach (var asset in value)
                {
                    if (groups.Contains(asset.owner.name))
                        continue;
                    groups.Add(asset.owner.name);
                }

                groups.Sort();
                var bundle = $"auto_{string.Join("_", groups)}_assets";
                entry.owner = group;
                entry.bundle = Settings.PackAsset(entry.asset, bundle);
