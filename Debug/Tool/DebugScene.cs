...
namespace JFramework.Logger
{
    internal class DebugScene
    {
        ...
        private void ShowGameObject()
        {
            ...
            GUILayout.BeginVertical(GUIStyles.Box, GUILayout.Width((debugData.MaxWidth - 25) / 2));
            GUILayout.BeginHorizontal();
            objectFilter = GUILayout.TextField(objectFilter, GUIStyles.TextField, GUIStyles.MinHeightFix);
            ...
            for (int i = 0; i < objectList.Count; i++)
            {
                ...
                if (GUILayout.Toggle(value, objectList[i].name, GUIStyles.Label) != value)
                {
                    if (objectIndex != i)
                    {
                        objectIndex = i;
                ...
                if (objectIndex == i)
                {
                    ...
                    GUILayout.BeginHorizontal();
                    GUILayout.Label("Tag: " + objectList[i].tag, GUIStyles.Label);
                    GUILayout.EndHorizontal();
                    
                    GUILayout.BeginHorizontal();
                    GUILayout.Label("Layer: " + LayerMask.LayerToName(objectList[i].gameObject.layer), GUIStyles.Label);
                    GUILayout.EndHorizontal();
#if ENABLE_PROFILER
                    GUILayout.BeginHorizontal();
                    GUILayout.Label("Memory" + ": " + objectMemory + "KB", GUIStyles.Label);
                    GUILayout.EndHorizontal();
#endif
        ...
        private void ShowComponent()
        {
            ...
            {
                GUILayout.BeginHorizontal();
                isAddComponent = GUILayout.Toggle(isAddComponent, debugData.GetData("Add Component"), GUIStyles.Button, GUIStyles.MinHeightFix);
                if (componentIndex != -1 && componentIndex < objectComponents.Count)
                {
                    if (objectComponents[componentIndex])
                    {
                        if (GUILayout.Button(debugData.GetData("Delete Component"), GUIStyles.Button, GUIStyles.MinHeightFix))
                        {
            ...
            if (objectIndex != -1)
            {
                ...
                {
                    GUILayout.BeginHorizontal();
                    componentFilter = GUILayout.TextField(componentFilter, GUIStyles.TextField, GUIStyles.MinHeightFix);
                    if (GUILayout.Button(debugData.GetData("Search"), GUIStyles.Button, GUIStyles.MinHeightFix))
                    {
                        addComponents.Clear();
                    ...
                    foreach (var addComponent in addComponents)
                    {
                        ...
                        if (fullName != null)
                        {
                            ...
                            if (GUILayout.Button(strArray[^1], GUIStyles.Button, GUIStyles.MinHeightFix))
                            {
                ...
                else
                {
                    for (int i = 0; i < objectComponents.Count; i++)
                    {
                        if (objectComponents[i])
                        {
                            ...
                            if (GUILayout.Toggle(value, objectComponents[i].GetType().Name, GUIStyles.Label) != value)
                            {
                                if (componentIndex != i)
                                {
                                    componentIndex = i;
                            ...
                            if (componentIndex == i)
                            {
                                ...
                                {
                                    GUILayout.Label("No Debug GUI!", GUIStyles.Label);
                                }
        ...
        {
            GUILayout.BeginHorizontal();
            GUILayout.Label(debugData.GetData("GameObjects") + " [" + objectList.Count + "]", GUIStyles.Button, GUIStyles.MinHeight);
            GUI.contentColor = Color.white;
            if (GUILayout.Button(debugData.GetData("Refresh"), GUIStyles.Button, GUILayout.Width((debugData.MaxWidth - 25) / 2), GUILayout.Height(60)))
            {
                RefreshGameObject();
                RefreshComponent();
                return;
