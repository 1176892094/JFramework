...
namespace JFramework
{
    internal class DebugScene
    {
        ...
        public DebugScene(DebugSetting setting) => this.setting = setting;
        ...
        private void ShowGameObject()
        {
            ...
            GUILayout.BeginVertical(DebugStyle.Box, GUILayout.Width((setting.MaxWidth - 25) / 2));
        ...
        private void ShowComponent()
        {
            ...
            {
                GUILayout.BeginHorizontal();
                isAddComponent = GUILayout.Toggle(isAddComponent, setting.GetData("Add Component"), DebugStyle.Button, DebugStyle.MinHeightFix);
                if (componentIndex != -1 && componentIndex < objectComponents.Count)
                {
                    if (objectComponents[componentIndex])
                    {
                        if (GUILayout.Button(setting.GetData("Delete Component"), DebugStyle.Button, DebugStyle.MinHeightFix))
                        {
            ...
            if (objectIndex != -1)
            {
                ...
                if (isAddComponent)
                {
                    ...
                    if (GUILayout.Button(setting.GetData("Search"), DebugStyle.Button, DebugStyle.MinHeightFix))
                    {
                        addComponents.Clear();
        ...
        {
            GUILayout.BeginHorizontal();
            GUILayout.Label(setting.GetData("GameObjects") + " [" + objectList.Count + "]", DebugStyle.Button, DebugStyle.MinHeight);
            GUI.contentColor = Color.white;
            if (GUILayout.Button(setting.GetData("Refresh"), DebugStyle.Button, GUILayout.Width((setting.MaxWidth - 25) / 2), GUILayout.Height(60)))
            {
                RefreshGameObject();
                RefreshComponent();
                return;
