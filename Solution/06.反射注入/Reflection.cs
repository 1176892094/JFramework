...
namespace JFramework
{
    internal static class Reflection
    {
        ...
        static Reflection()
        {
            ...
            toolbarRoot = toolbarType.GetField("m_Root", Service.Find.Entity);
            activeHierarchy = hierarchyType.GetField("m_SceneHierarchy", Service.Find.Entity);
            selectHierarchy = hierarchyType.GetField("s_LastInteractedHierarchy", Service.Find.Static);
            inspectorTracker = inspectorType.GetProperty("tracker", Service.Find.Entity);

            var editorType = Service.Find.Assembly("UnityEditor.CoreModule").GetType("UnityEditor.UIElements.EditorElement");
            editorElements = editorType.GetProperty("m_Editors", Service.Find.Entity);
            editorType = GetEditorType("SceneHierarchy");
            hierarchyTree = editorType.GetField("m_TreeView", Service.Find.Entity);
            editorType = GetEditorType("IMGUI.Controls.TreeViewGUI");
            iconWidth = editorType.GetField("k_IconWidth", Service.Find.Entity);
            iconLabel = editorType.GetField("k_SpaceBetweenIconAndText", Service.Find.Entity);
            ...
            itemTreeViewA = editorType.GetField("m_AssetTree", Service.Find.Entity);
            itemTreeViewB = editorType.GetField("m_FolderTree", Service.Find.Entity);
            editorType = GetEditorType("IMGUI.Controls.TreeViewController");
            itemTreeViewC = editorType.GetProperty("gui", Service.Find.Entity);
            itemTreeViewD = editorType.GetProperty("data", Service.Find.Entity);
            editorType = GetEditorType("ProjectBrowserColumnOneTreeViewDataSource");
            itemTreeViewE = editorType.GetMethod("GetRows", Service.Find.Entity);
            editorType = GetEditorType("AssetsTreeViewDataSource");
            itemTreeViewF = editorType.GetMethod("GetRows", Service.Find.Entity);
