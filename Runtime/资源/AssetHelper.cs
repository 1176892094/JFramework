...
namespace JFramework
{
    ...
    internal static class AssetHelper
    {
        ...
        internal static async Task UpdateAsync()
        {
            ...
            if (success)
            {
                var serverInfo = await File.ReadAllTextAsync(AssetSetting.serverInfoPath);
        ...
        private static async Task<bool> LoadServerAssetBundleInfo()
        {
            ...
            {
                success = await Download(AssetSetting.clientInfoName, AssetSetting.serverInfoPath);
            }
        ...
        private static async Task<bool> LoadAssetBundles()
        {
            ...
            while (assetDataList.Count > 0 && reloads-- > 0)
            {
                foreach (var fileName in copies)
                {
                    ...
                    var success = await Download(fileName,  AssetSetting.GetPerFile(fileName));
                    if (!success) continue;
        ...
        {
            using var request = UnityWebRequest.Get(AssetSetting.GetRemoteFile(fileName));
