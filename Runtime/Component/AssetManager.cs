...
namespace JFramework.Core
{
    public static class AssetManager
    {
...
#if UNITY_EDITOR
            if (!AssetSetting.isRemote) return AssetSetting.Instance.Load<T>(path);
#endif
        ...
        public static Task<T> LoadAsync<T>(string path) where T : Object
        {
            if (!GlobalManager.Runtime) return null;
#if UNITY_EDITOR
            if (!AssetSetting.isRemote) return AssetSetting.Instance.LoadAsync<T>(path);
#endif
            if (assets.TryGetValue(path, out var asset))
            {
        ...
        internal static Task<AsyncOperation> LoadSceneAsync(string path)
        {
            if (!GlobalManager.Runtime) return null;
#if UNITY_EDITOR
            if (!AssetSetting.isRemote) return AssetSetting.Instance.LoadSceneAsync(path);
#endif
            if (assets.TryGetValue(path, out var asset))
            {
