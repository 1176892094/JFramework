...
public class HierarchyEditor
{
    ...
    private void InternalReflection()
    {
        MethodInfo DisplayObjectContextMenu = typeof(EditorUtility).GetMethods(BindingFlags.Static | BindingFlags.NonPublic).Single(
            method => method.Name == nameof(DisplayObjectContextMenu) && method.GetParameters()[1].ParameterType == typeof(Object));
    ...
    private void HierarchyOnGUI(int selectionID, Rect selectionRect)
    {
        ...
        Rect rect = new Rect(selectionRect) { x = 34, width = 16 };
    ...
    private void ShowComponents()
    {
        ...
        for (int i = 0; i < length; ++i)
        {
            ...
            Rect rect = GetRectChanged(rowItem.nameRect, 12, ref afterName);
            if (hasMaterial && i == length - rendererComponent.sharedMaterials.Length)
            {
                foreach (var sharedMaterial in rendererComponent.sharedMaterials)
                {
                    ...
                    rect = GetRectChanged(rowItem.nameRect, 12, ref afterName);
                }
    ...
    private Rect GetRectChanged(Rect rect, float width, ref float usedWidth)
    {
        rect.xMin = 0;
        rect.x += usedWidth;
        rect.width = width;
